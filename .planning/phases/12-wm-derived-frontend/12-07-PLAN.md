...
<task type="auto">
  ...
  <action>
    ...
    **main.ts** (~120-160 lines): Complete rewrite of the stub from Plan 01. Bootstrap sequence:
    ...
    10. Create modals (append to body, hidden initially):
        ...
        ```
    11. Wire data flow -- initial load + event handlers:
        - Fetch initial data: `forecastClient.getTopForecasts()`, `forecastClient.getCountries()`, `forecastClient.getHealth()`
        - Feed data to panels and map:
          ```typescript
          ...
          ```
        - Wire 'forecast-selected' event: update EnsembleBreakdownPanel and CalibrationPanel with selected forecast's data
        - Wire 'country-selected' event: update map highlight + open CountryBriefPage
    12. Set up RefreshScheduler:
        ```typescript
        const scheduler = new RefreshScheduler();
        scheduler.register([
          { name: 'forecasts', fn: async () => { /* fetch + update */ }, intervalMs: 60_000 },
          { name: 'countries', fn: async () => { /* fetch + update */ }, intervalMs: 120_000 },
          { name: 'health', fn: async () => { /* fetch + update */ }, intervalMs: 30_000 },
        ]);
        scheduler.start();
        ```
    ...
    **Critical wiring to get right:**
    ...
    - EnsembleBreakdownPanel updates when a forecast is selected (via same 'forecast-selected' event)
    - CalibrationPanel can update from selected forecast's calibration data
  ...
  <done>
    ...
    Initial data load populates panels and map.
    RefreshScheduler periodically updates data.
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
    ...
    - Country Brief Page with 6 tabs (Overview, Active Forecasts, GDELT Events, Forecast History, Entity Relations, Calibration)
  ...
  <how-to-verify>
    ...
    9. Verify: Click a country on the globe -> Country Brief modal opens
    10. Verify: Country Brief has tabs, switch between them
...
- SC-4: Country Brief with 4+ tabs populated from API
- SC-5: RiskIndexPanel with trend indicators
- SC-6: CalibrationPanel with reliability diagram + Brier decomposition
