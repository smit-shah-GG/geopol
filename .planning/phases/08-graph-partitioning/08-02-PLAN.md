...
must_haves:
  truths:
    ...
    - "Query router prioritizes home partition for boundary entities"
  ...
  key_links:
    ...
    - from: "src/knowledge_graph/cross_partition_query.py"
      to: "src/knowledge_graph/boundary_resolver.py"
      via: "home partition lookup for query prioritization"
      pattern: "BoundaryResolver|get_home_partition|boundary_entities"
    - from: "src/knowledge_graph/partitioned_graph.py"
...
Purpose: This plan enables query correctness across partition boundaries. The boundary resolver identifies entities appearing in multiple partitions and marks their home/replica status. The query router implements scatter-gather to execute queries across relevant partitions and merge results, prioritizing home partitions for boundary entities. The PartitionedTemporalGraph provides a drop-in interface matching the existing GraphTraversal API.
...
2. **BoundaryResolver class**:
   ...
   - `get_home_partition(entity_id: str) -> Optional[str]`:
     - Return home partition for entity, or None if not a boundary entity
     - This method is critical for QueryRouter integration
...
5. **Tests** in `tests/test_boundary_resolver.py`:
   ...
   - test_get_home_partition: Returns correct home for boundary entity, None for non-boundary
  </action>
  <verify>
```bash
...
All tests pass.
  ...
  <done>
BoundaryResolver identifies cross-partition entities, calculates home/replica assignments, provides boundary statistics. get_home_partition() method available for QueryRouter integration.
  </done>
</task>

<task type="auto">
  <name>Task 2a: Implement scatter-gather query router with BoundaryResolver integration</name>
  <files>src/knowledge_graph/cross_partition_query.py, tests/test_cross_partition_query.py</files>
  <action>
Create `src/knowledge_graph/cross_partition_query.py` with QueryRouter class:

1. **QueryRouter class** (from RESEARCH.md Pattern 4):
   - `__init__(self, manager: PartitionManager, index: EntityPartitionIndex, boundary_resolver: BoundaryResolver, max_workers: int = 4)`
     - **NOTE:** BoundaryResolver is a required parameter for home partition prioritization
   ...
   - `execute_k_hop_query(entity_id, k, time_start, time_end) -> TraversalResult`:
     ...
     - **Step 3 (NEW): Prioritize home partition for boundary entities**
       - Call `boundary_resolver.get_home_partition(entity_id)`
       - If entity is boundary, reorder partitions to query home first
       - This reduces unnecessary duplicate work on replicas
     - Step 4: Scatter - submit parallel queries to each partition
     - Step 5: Gather - collect results, merge into single TraversalResult
...
3. **Tests** in `tests/test_cross_partition_query.py`:
   ...
   - **test_home_partition_priority**: Verify that for boundary entities, home partition appears first in query order (mock or inspect call order)
  </action>
  <verify>
```bash
cd /home/kondraki/personal/geopol && python -m pytest tests/test_cross_partition_query.py -v
```
All tests pass, including test_home_partition_priority.
  </verify>
  <done>
QueryRouter implements parallel scatter-gather with BoundaryResolver integration. Home partition is prioritized for boundary entity queries.
  </done>
</task>

<task type="auto">
  <name>Task 2b: Implement PartitionedTemporalGraph with query correctness tests</name>
  <files>src/knowledge_graph/partitioned_graph.py, tests/test_partitioned_graph.py</files>
  <action>
Create `src/knowledge_graph/partitioned_graph.py` with PartitionedTemporalGraph class:
...
1. **PartitionedTemporalGraph class**:
   - `__init__(self, partition_dir: Path, index_path: Path, max_cached: int = 4)`:
     ...
     - **Create BoundaryResolver with index and manager**
     - **Create QueryRouter with manager, index, AND boundary_resolver**
...
3. **Tests** in `tests/test_partitioned_graph.py`:
   ...
   - test_stats_include_boundary_info: get_stats() includes boundary entity statistics
  </action>
  <verify>
```bash
cd /home/kondraki/personal/geopol && python -m pytest tests/test_partitioned_graph.py -v
```
All tests pass, especially test_query_correctness_vs_full_graph.
  </verify>
  <done>
PartitionedTemporalGraph provides unified interface with full BoundaryResolver wiring. Query correctness test proves equivalence with single-graph queries.
  </done>
</task>
...
- [ ] BoundaryResolver.get_home_partition() returns correct home partition
- [ ] QueryRouter accepts BoundaryResolver in constructor
- [ ] QueryRouter prioritizes home partition for boundary entity queries
- [ ] PartitionedTemporalGraph wires BoundaryResolver to QueryRouter
...
- [ ] test_home_partition_priority passes
- [ ] All tests pass
