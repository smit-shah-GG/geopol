...
must_haves:
  truths:
    - "GET /api/v1/health returns JSON with all 8 subsystem statuses (database, redis, gdelt_store, graph_partitions, tkg_model, last_ingest, last_prediction, api_budget) without requiring authentication"
  ...
  artifacts:
    ...
    - path: "src/api/routes/v1/health.py"
      provides: "Full subsystem inventory health endpoint (8 checks)"
      contains: "graph_partitions"
  ...
  key_links:
    ...
    - from: "src/api/routes/v1/health.py"
      ...
      via: "Partition count from EntityPartitionIndex or direct SQLite query"
      pattern: "partition|EntityPartitionIndex"
---

<objective>
Build the FastAPI application with all routes, middleware, and error handling — serving mock fixture data through the DTO contract established in Plan 02. The health endpoint provides a full subsystem inventory (8 checks) that serves double duty as ops health probe AND data source for Phase 12's SystemHealthPanel.
...
Output: `uvicorn src.api.app:create_app --factory` starts a server where all endpoints return valid responses. Health endpoint is public and reports all 8 subsystem statuses. Forecast endpoints require API key auth. OpenAPI docs auto-generated at /docs.
</objective>
...
# Partition index for health check:
...
<task type="auto">
  <name>Task 2: API routes — full subsystem health, forecasts, countries</name>
  <files>
    src/api/routes/__init__.py
    src/api/routes/v1/__init__.py
    src/api/routes/v1/router.py
  ...
  <action>
    ...
    2. Create src/api/routes/v1/health.py — FULL SUBSYSTEM INVENTORY (8 checks):
       - GET /health (no auth required)
       - Returns HealthResponse with ALL 8 subsystem statuses:

         a. "database": attempt async SELECT 1 on PostgreSQL, report healthy/unhealthy with connection pool stats in detail
         b. "redis": attempt PING on Redis, report healthy/unhealthy (gracefully handle Redis not available — degraded, not crash)
         c. "gdelt_store": check if GDELT SQLite file exists at settings.GDELT_DB_PATH, verify PRAGMA journal_mode is WAL. Report file size in detail.
         d. "graph_partitions": query partition count from the partition index SQLite database (same file or adjacent to GDELT store). Import EntityPartitionIndex from src.knowledge_graph.partition_index if available, or read the partition_index table directly via SQLite. Report partition count in detail (e.g., "12 partitions"). If no partitions exist yet, report healthy=True with detail="0 partitions (not yet populated)".
         e. "tkg_model": check if a TKG model checkpoint file exists on disk (look for data/model_checkpoint/ or similar path from settings or hardcoded default). Report model path and file size if found. healthy=True if file exists, healthy=False if not. Phase 11 will update the path.
         f. "last_ingest": query the most recent row from ingest_runs table (created_at DESC LIMIT 1). Return the timestamp in detail. If no rows exist (table empty — Phase 10 hasn't run yet), return healthy=True with detail="no ingest runs yet" — absence of data is NOT an error at this phase.
         g. "last_prediction": query the most recent row from predictions table (created_at DESC LIMIT 1). Return the timestamp in detail. Same null-handling as last_ingest.
         h. "api_budget": placeholder/stub — return healthy=True with detail="budget tracking not implemented (Phase 10)". This will be wired to Gemini API rate limiting in Phase 10.

       - Overall status logic:
         - "healthy" if ALL subsystems healthy
         - "degraded" if any subsystem unhealthy but database is healthy (system partially functional)
         - "unhealthy" if database is unhealthy (nothing works without PostgreSQL)
       - Include timestamp and version ("2.0.0-dev") in response

       - IMPORTANT: Each subsystem check must be wrapped in try/except. A failing check should report healthy=False with the error message in detail — it must NEVER crash the health endpoint. The health endpoint must always return 200 with the inventory.
  ...
  <verify>
    ...
    # Health (no auth) — verify all 8 subsystems present
...
expected = ['database','redis','gdelt_store','graph_partitions','tkg_model','last_ingest','last_prediction','api_budget']
for e in expected:
    assert e in names, f'Missing subsystem: {e}'
print(f'All 8 subsystems present: {names}')
"
    ...
    Expected results:
    - Health: 200 with 8 subsystem statuses (some may be unhealthy if services not running — that's fine, the endpoint itself must not crash)
  ...
  <done>All API endpoints return valid responses. Health endpoint reports all 8 subsystem statuses without requiring auth. Forecast/country endpoints require API key. POST without key returns 401. OpenAPI schema auto-generated with all DTOs. RFC 9457 error format on all errors.</done>
</task>

</tasks>

<verification>
1. `GET /api/v1/health` returns 200 with all 8 subsystem statuses
...
7. Health endpoint never crashes — failing checks report unhealthy with error detail
</verification>

<success_criteria>
- FastAPI server starts with `uvicorn src.api.app:create_app --factory`
- All endpoints return structurally valid DTO responses
- Health endpoint reports 8 subsystem statuses (database, redis, gdelt_store, graph_partitions, tkg_model, last_ingest, last_prediction, api_budget)
