...
<task type="auto">
  ...
  <action>
    ...
    4. Create src/api/schemas/health.py:
       - SubsystemStatus: name (str), healthy (bool), detail (Optional[str])
       - HealthResponse: status (Literal["healthy", "degraded", "unhealthy"]), subsystems (list[SubsystemStatus]), timestamp (datetime)
...
"` succeeds without errors
  ...
  <done>All 6 DTOs plus ProblemDetail, PaginatedResponse, and HealthResponse are importable and validate correctly. ScenarioDTO handles recursive nesting. ForecastResponse generates valid OpenAPI schema.</done>
</task>
