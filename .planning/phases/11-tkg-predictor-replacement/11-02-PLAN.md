...
must_haves:
  truths:
    ...
    - "Training uses mixed precision (bfloat16 forward, float32 params); evolve_embeddings() applies jax.checkpoint per snapshot for gradient memory efficiency"
    ...
    - "compare_models() loads best RE-GCN checkpoint, evaluates on same held-out split as TiRGN, and writes comparison_report.json with tirgn_mrr, regcn_mrr, delta, and pass/fail status"
  artifacts:
    ...
    - path: "scripts/train_tirgn.py"
      ...
      min_lines: 50
    - path: "src/training/compare_models.py"
      provides: "compare_models() -- loads RE-GCN and TiRGN best checkpoints, evaluates both on held-out test split, writes comparison_report.json"
      min_lines: 60
    - path: "tests/test_train_tirgn.py"
      provides: "Unit tests for training loop components (data loading, history vocab construction, early stopping logic, logger, comparison report)"
      min_lines: 80
  key_links:
    ...
    - from: "scripts/train_tirgn.py"
      ...
      pattern: "train_tirgn"
    - from: "src/training/compare_models.py"
      to: "src/training/train_jax.py"
      via: "Reuses load_checkpoint(), compute_mrr() to evaluate RE-GCN on same held-out split"
      pattern: "from src.training.train_jax import"
---
...
<task type="auto">
  ...
  **tests/test_train_tirgn.py**:
  ...
  <done>
  TiRGN training loop is runnable with early stopping, mixed precision, VRAM monitoring, TensorBoard + optional W&B logging. CLI entry point accepts all hyperparameters. Unit tests verify training infrastructure without GPU dependency. TKG-04 (GPU envelope with mixed precision + gradient checkpointing) is structurally covered.
  </done>
</task>

<task type="auto">
  <name>Task 3: RE-GCN baseline evaluation and MRR comparison report</name>
  <files>
    src/training/compare_models.py
    tests/test_train_tirgn.py
  </files>
  <action>
  Build the comparison infrastructure that satisfies TKG-03: "TiRGN implementation achieves measurable MRR improvement over RE-GCN baseline on held-out GDELT test set."

  **src/training/compare_models.py** -- `compare_models()`:

  1. `ComparisonResult(dataclass)`:
     - `tirgn_mrr: float`
     - `regcn_mrr: float`
     - `delta: float` (tirgn_mrr - regcn_mrr)
     - `delta_pct: float` ((delta / regcn_mrr) * 100)
     - `pass_threshold: float` (default -5.0, i.e. TiRGN ships if within 5% of RE-GCN per CONTEXT.md failure strategy)
     - `passed: bool` (delta_pct >= pass_threshold)
  ...
  2. `evaluate_regcn_baseline(data_path: Path, regcn_checkpoint: Path, max_events: int = 0, num_days: int = 30) -> float`:
     - Load RE-GCN checkpoint via `load_checkpoint()` from `src.training.train_jax` (reuse existing code, do NOT reimplement).
     - Load GDELT data via `load_gdelt_data()` (reuse from train_jax).
     - Extract the SAME held-out test split that TiRGN uses. The split is determined by `load_gdelt_data()` -- it returns `val_triples` which serves as the held-out set. Both models MUST be evaluated on the same `val_triples`.
     - Compute MRR via `compute_mrr()` from `src.training.train_jax` (reuse).
     - Return MRR as float.

  3. `compare_models(data_path: Path, tirgn_checkpoint: Path, regcn_checkpoint: Path, output_path: Path, max_events: int = 0, num_days: int = 30) -> ComparisonResult`:
     - Load GDELT data ONCE (shared between both evaluations).
     - Evaluate TiRGN: load TiRGN checkpoint, compute MRR on held-out test split using the same `compute_mrr()` function (requires adapting call signature since TiRGN uses `evolve_embeddings()` + `compute_scores()` pattern, not REGCN's direct interface -- write a thin adapter `_tirgn_compute_mrr()` that wraps TiRGN's predict path into the same evaluation protocol).
     - Evaluate RE-GCN: call `evaluate_regcn_baseline()`.
     - Compute delta and pass/fail.
     - Write `comparison_report.json` to `output_path`:
       ```json
       {
         "tirgn_mrr": 0.452,
         "regcn_mrr": 0.404,
         ...
         "regcn_checkpoint": "models/tkg/regcn_trained.npz",
         "eval_split": "test",
         "num_test_triples": 12345,
         "timestamp": "2026-03-01T12:00:00Z"
       }
       ```
     - Log comparison summary to stdout: "TiRGN MRR: {:.4f} | RE-GCN MRR: {:.4f} | Delta: {:.4f} ({:.1f}%) | {PASS/FAIL}".
     - Return ComparisonResult.

  4. CLI block at bottom of file (`if __name__ == "__main__"`):
     - argparse with: `--data-path`, `--tirgn-checkpoint`, `--regcn-checkpoint`, `--output` (default: `models/tkg/comparison_report.json`), `--max-events`, `--num-days`.
     - Calls `compare_models()` and exits with code 0 if passed, 1 if failed.

  **tests/test_train_tirgn.py** (add to existing tests from Task 2):

  6. `test_comparison_result_dataclass`: Verify ComparisonResult fields and pass/fail logic (delta_pct >= pass_threshold -> passed=True).
  7. `test_comparison_report_json_schema`: Mock both model evaluations, call compare_models(), verify output JSON contains all required keys.

  **CRITICAL**: The held-out split MUST be identical for both models. The split comes from `load_gdelt_data()` which is called once -- both models are evaluated on the same `val_triples` array. No re-splitting allowed.
  </action>
  <verify>
  `python -c "from src.training.compare_models import compare_models, ComparisonResult; print('import OK')"` succeeds.
  `python -c "
from src.training.compare_models import ComparisonResult
r = ComparisonResult(tirgn_mrr=0.45, regcn_mrr=0.40, delta=0.05, delta_pct=12.5, pass_threshold=-5.0, passed=True, tirgn_checkpoint='t.npz', regcn_checkpoint='r.npz', eval_split='test', num_test_triples=100, timestamp='2026-03-01')
assert r.passed == True
r2 = ComparisonResult(tirgn_mrr=0.35, regcn_mrr=0.40, delta=-0.05, delta_pct=-12.5, pass_threshold=-5.0, passed=False, tirgn_checkpoint='t.npz', regcn_checkpoint='r.npz', eval_split='test', num_test_triples=100, timestamp='2026-03-01')
assert r2.passed == False
print('comparison logic OK')
"` succeeds.
  `uv run pytest tests/test_train_tirgn.py -v -k compare` -- comparison tests pass.
  </verify>
  <done>
  compare_models() evaluates both TiRGN and RE-GCN on the identical held-out test split, computes MRR delta, writes comparison_report.json as a durable artifact with all metrics. Requirement TKG-03 (measurable MRR improvement, logged and reproducible) is fully covered.
  </done>
</task>
...
4. `python -c "from src.training.compare_models import compare_models, ComparisonResult"` -- import succeeds.
5. `python scripts/train_tirgn.py --help` -- shows CLI usage with all expected arguments.
6. `uv run pytest tests/test_train_tirgn.py -v` -- all tests pass (including comparison tests).
7. TensorBoard logdir structure is created when logger is instantiated.
8. No modifications to existing train_jax.py (only imports from it).
</verification>
...
- compare_models() evaluates both TiRGN and RE-GCN on the same held-out split and produces comparison_report.json with tirgn_mrr, regcn_mrr, delta, and pass/fail status.
- All unit tests pass without GPU.
- Requirements TKG-03 (MRR comparison) and TKG-04 (GPU envelope) are fully covered.
</success_criteria>
