...
must_haves:
  truths:
    - "Running scripts/bootstrap.py --dry-run shows 5 stages that would execute"
    - "Running scripts/bootstrap.py completes and system can accept forecast queries"
  ...
  key_links:
    ...
    - from: "src/bootstrap/stages.py"
      ...
      via: "import EventStorage for Parquet-to-SQLite bridge"
      pattern: "from.*storage.*import.*EventStorage"
    - from: "src/bootstrap/stages.py"
      to: "src/database/models.py"
      via: "import Event model"
      pattern: "from.*models.*import.*Event"
---
...
- **Post-processing step:** Load parquet into EventStorage (SQLite) for graph builder.
  ...
  **IMPORTANT:** The Event model has `from_gdelt_row()` NOT `from_tkg_row()`. Use direct dict-based Event construction:
  ...
  for _, row in df.iterrows():
      ...
      # Create Event directly from parquet columns
      event = Event(
          content_hash=row_dict.get('content_hash') or hashlib.md5(
              f"{row_dict.get('event_date', '')}{row_dict.get('actor1_code', '')}{row_dict.get('actor2_code', '')}".encode()
          ).hexdigest(),
          time_window=row_dict.get('time_window') or row_dict.get('event_date', '')[:7],  # YYYY-MM
          event_date=row_dict.get('event_date', ''),
          gdelt_id=row_dict.get('gdelt_id'),
          actor1_code=row_dict.get('actor1_code'),
          actor2_code=row_dict.get('actor2_code'),
          event_code=row_dict.get('event_code'),
          quad_class=row_dict.get('quad_class'),
          goldstein_scale=row_dict.get('goldstein_scale'),
          num_mentions=row_dict.get('num_mentions'),
  ...
  This bridges the Parquet->SQLite gap identified in research without relying on non-existent methods.
...
- ProcessEventsStage includes Parquet->SQLite bridge using direct Event construction
