...
<task type="auto">
  ...
  **src/ingest/gdelt_poller.py** -- the main daemon class:
  ...
  5. `async _poll_once()` method:
     ...
     - If graph is available: for each new event, call `await asyncio.to_thread(self.graph.add_event_from_db_row, row_dict)` -- incremental O(N_new) update per INGEST-02.
  ...
  **deploy/systemd/geopol-gdelt-poller.service**:
  ...
  <verify>
  `python -c "from src.ingest.gdelt_poller import GDELTPoller, BackoffStrategy; print('OK')"` succeeds. `uv run pytest tests/test_gdelt_poller.py -v` passes (tests from Task 3). `python scripts/gdelt_poller.py --help` shows usage.
  </verify>
  <done>
  GDELT poller daemon is runnable, handles SIGTERM gracefully, retries with exponential backoff, tracks per-run metrics in IngestRun, and updates knowledge graph incrementally. Backfill module detects gaps on startup.
  </done>
</task>
...
4. `uv run pytest tests/test_gdelt_poller.py -v` -- all tests pass.
5. `python scripts/gdelt_poller.py --help` shows CLI usage.
6. `deploy/systemd/geopol-gdelt-poller.service` file exists with correct configuration.
</verification>
