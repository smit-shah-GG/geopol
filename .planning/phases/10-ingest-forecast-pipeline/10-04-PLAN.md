...
must_haves:
  ...
  key_links:
    ...
    - from: "src/pipeline/daily_forecast.py"
      ...
      via: "RSS article ChromaDB collection available for RAG context during EnsemblePredictor.predict()"
      pattern: "rss_articles|ChromaDB|RAG"
---
...
@.planning/phases/10-ingest-forecast-pipeline/10-03-SUMMARY.md
...
<task type="auto">
  ...
  **deploy/systemd/geopol-daily-forecast.timer**:
  ...
  `systemd-analyze verify deploy/systemd/geopol-daily-forecast.timer 2>/dev/null || echo "systemd-analyze not available -- manual syntax review required"` -- validates timer syntax if systemd-analyze is present on the build host.
  </verify>
  <done>
  Daily pipeline orchestrates the full forecast cycle: dequeue carryover -> generate questions -> predict+persist -> resolve outcomes. Handles budget exhaustion by queuing overflow. Consecutive failure alerting. systemd timer runs at 06:00 daily with validated timer syntax.
  </done>
</task>

<task type="auto">
  <name>Task 3: Wire real API endpoints with cache, rate limit, and sanitization</name>
  <files>
    src/api/routes/v1/forecasts.py
    src/api/app.py
  </files>
  ...
  **src/api/routes/v1/forecasts.py** -- update existing routes to serve real data:
  ...
  1. `GET /forecasts/country/{iso_code}`:
     - Check ForecastCache first (cache key: `cache_key_for_country(iso_code)`).
  ...
  2. `GET /forecasts/top`:
     - Check ForecastCache (cache key: `f"forecast:top:{limit}"`).
  ...
  3. `GET /forecasts/{forecast_id}`:
     - Already queries PostgreSQL first (Phase 9 wired this). Add cache layer:
       - Check ForecastCache (cache key: `cache_key_for_forecast(forecast_id)`).
  ...
  **src/api/app.py** -- update lifespan:
  ...
  <done>
  API routes serve real forecasts with three-tier cache layer, rate limiting, and input sanitization. Mock fixtures preserved as fallback. Redis lifecycle managed in app lifespan.
  </done>
</task>

<task type="auto">
  <name>Task 4: Tests for daily pipeline and route wiring</name>
  <files>
    tests/test_daily_pipeline.py
  </files>
  <action>
  Write tests for pipeline components and route wiring. Mock all external dependencies (Gemini, PostgreSQL, Redis, EventStorage).
  ...
  **Route wiring tests:**
  ...
  <done>
  12 tests cover pipeline components (7: question gen, budget tracking, outcome resolution, pipeline lifecycle, consecutive failure alerting) and route wiring (5: cache hit/miss, rate limiting, sanitization, budget exhaustion). All pass without external dependencies.
  </done>
</task>
...
3. `uv run pytest tests/test_daily_pipeline.py -v` -- all 12 tests pass.
