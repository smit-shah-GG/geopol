import type { GeoPolAppContext, AppModule } from '@/app/app-context';
...
/**
 * Periodic data refresh scheduler with visibility-aware throttling.
 *
 * Adapted from WM's RefreshScheduler. Key behaviors:
 * - Background tabs get 4x slower refresh to save bandwidth
 * - Jitter prevents thundering herd on tab restore
 * - In-flight dedup prevents overlapping requests for same resource
 * - Stale refresh flush on tab visibility restore
 */
export class RefreshScheduler implements AppModule {
  private ctx: GeoPolAppContext;
  ...
  constructor(ctx: GeoPolAppContext) {
    this.ctx = ctx;
  }
  ...
  scheduleRefresh(
    ...
    const run = async () => {
      ...
      try {
        ...
        console.error(`[Geopol] Refresh ${name} failed:`, e);
      } finally {
