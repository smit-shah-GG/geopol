#!/usr/bin/env python
"""
Train RE-GCN temporal knowledge graph model using JAX.

Memory-efficient implementation using:
- jax.checkpoint for gradient checkpointing
- Flax NNX for explicit state management
- Optax for optimization

Usage:
    uv run python scripts/train_tkg_jax.py
    uv run python scripts/train_tkg_jax.py --max-events 200000 --epochs 50
    uv run python scripts/train_tkg_jax.py --dry-run
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train RE-GCN temporal knowledge graph model using JAX"
    )
    parser.add_argument(
    ...
    if args.dry_run:
        ...
        from src.training.train_jax import load_gdelt_data, create_graph_snapshots
        ...
        # Test forward pass
        snapshots = create_graph_snapshots(snapshots_np, len(relation_to_id))
        import jax.numpy as jnp
        test_triple = jnp.array([[0, 0, 1]], dtype=jnp.int32)
        scores = model.predict(snapshots, test_triple)
    ...
    # Full training
    from src.training.train_jax import train_regcn, TrainingConfig
    ...
    try:
        ...
        logger.info(f"  Model saved to: {MODEL_DIR}/regcn_trained.npz")
